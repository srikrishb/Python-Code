...
def createMap(key, eachItem):
    temp_map = {}
    temp_map[key] = eachItem
    return temp_map
    ...
    for line in deltaListDataRow:
        if line[0].endswith('.bpmn'):
            ...
            if resourceId != 'Workflow Not Found':
                ...
                referenceDataFile = 'referenceData.json'

                # Open the itemResourceType file and find the itemResourceTypeCode
                os.chdir("K:\Git Code\Python\ParameterFiles")
                with open(referenceDataFile) as jsonfile:
                    referenceData = json.load(jsonfile)
                    itemResourceTypeRefCode = referenceData['itemResourceType'][appliesTo]
                    startEventRefCode = referenceData['startEvent']

                # Define endpoint and payload
                appliesToEndpoint = "workflow/" + resourceId + "/itemResourceType"
                appliesToPayload = {'itemResourceType': itemResourceTypeRefCode}
                ...
                if 'startEvent' in paramFileData != '':
                    ...
                    prev_value = 'eventTypes='
                    startEventPayload = ''
                    for eachEvent in paramFileData['startEvent']:
                        startEventPayload = prev_value + startEventRefCode[eachEvent]
                        prev_value = startEventPayload + '&eventTypes='
                ...
                # Other Variables
                otherVariableEndpoint = "workflow/" + resourceId
                otherVariablePayload = paramFileData['Other']
                otherVariableResponse = workflowPerformPostSessionDataCall(otherVariableEndpoint, otherVariablePayload)

                if otherVariableResponse != '1':
                    print('Error while changing configuration variables of the workflow. Please check logs.')
                    break
                elif otherVariableResponse == '1':
                    print('Workflow configuration variables set successfully.')
